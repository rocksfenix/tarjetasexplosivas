...
const ConfettiGenerator = function (params) {
  ...
  var _render = function () {
    ...
    function draw () {
      ...
      // animation loop
      if (appstate.animate) window.requestAnimationFrame(draw)
    }
...
export default class extends React.Component {
  ...
  componentDidMount () {
    if (process.browser) {
      ...
      var confetti = ConfettiGenerator({ target: 'confi', size: 1.8, max: 60 })
      confetti.render()
    }
  }
  render () {
    return (
      <canvas id='confi' width={this.state.width} height={this.state.height} style={{ position: 'fixed', top: '0' }} />
    )
  }
}
